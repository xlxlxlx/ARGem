...
if __name__=="__main__":
    ...
    for i in range(0,df.shape[0]):
        ...
        if "mobileOG" in mge_line:
            mge_label = mge_line.split("|")[1]  
        elif len(mge_line.split("|")) > 3:
        ...
        if count > threshold:   
           if arg_line not in list_of_ARG.keys():
                ...
                nodes.append(temp)
                #ARG bipartite = 0
                arg_label = arg_line.split("|")[3]
                G.add_node(arg_label, bipartite = 0)
                G_arg.append(arg_label)
           if mge_line not in list_of_MGE.keys():
               ...
               nodes.append(temp)
               #MGE bipartite = 1
               G.add_node(mge_label, bipartite = 1)
           ...
           G.add_edge(arg_label, mge_label) 
    ...
    #print(data)
    print(G.nodes)
    pos = bipartite_layout(G, G_arg)
    #pos = nx.shell_layout(G)
    fig = matplotlib.pyplot.figure(figsize=(30,30))
    nx.draw_networkx(G, pos, with_labels=True, ax=fig.add_subplot())
    matplotlib.use("Agg")
    fig.savefig("networkx_"+output +".png")
