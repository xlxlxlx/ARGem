...
class BlastnDriver:
    ...
    def run_blastn(self):
        ...
        for assembly_folder in assembly_folders:
            ...
            # run it!
            # NOTE: blastn can only use upto 40 threads, which is the reason why num_threads option for blastn is 40
        ...
        #TODO check if the run pipline create annotations
        with open(f'{self.proj_path}/annotations/log.txt', 'w') as log:
...
if __name__=="__main__":
    ...
    blastnDvr = BlastnDriver(proj_path)
    blastnDvr.run_blastn()
    #blastnDvr.count_combinations()
