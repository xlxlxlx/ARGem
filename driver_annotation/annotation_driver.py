...
class AnnotationDriver:
    def __init__(self, proj_path):
        ...
        self.mobileOG_dmnd_path = "driver_annotation/databases/mobileOG.dmnd"
        self.assembly_path = f"{proj_path}/assembly_output"


    def run_annotation(self, database):
        # for logging
        ...
        for assembly_folder in assembly_folders:
            ...
            # run it!
            print("====== mge database: " + database + "======")
            print(f'{self.diamond_path}/diamond blastx -d {self.metacompare_dmnd_path} --query {self.assembly_path}/{assembly_folder}/final.contigs.fa --out {out_path}/out_metacompare.tsv --outfmt 6 --threads 64 --evalue 1e-10')
            print(database is "metacompare")
            ...
            if (database == "larsson"):
                print(f'{self.blastn_path}/blastn -db {self.larsson_path} -query {to_annotate}/{contig} -out {out_path}/out_larsson.tsv -outfmt 6 -num_threads 40 -evalue 1e-10')
                subprocess.run(f'{self.blastn_path}/blastn -db {self.larsson_path} -query {to_annotate}/{contig} -out {out_path}/out_larsson.tsv -outfmt 6 -num_threads 40 -evalue 1e-10', shell=True)            
            elif (database == "metacompare"):
            ...
            # (TODO): double check how mobileOG runs
            elif (database == "mobileOG"):
                subprocess.run(f'{self.diamond_path}/diamond blastx -d {self.mobileOG_dmnd_path} --query {self.assembly_path}/{assembly_folder}/final.contigs.fa --out {out_path}/out_mobileOG.tsv --outfmt 6 --threads 64 --evalue 1e-10', shell=True)
            else:
                print("Invalid database input")
        ...
        # temp fix
        if not os.path.isdir(f'{self.proj_path}/annotations'):
            os.makedirs(f'{self.proj_path}/annotations')
    ...
    def count_combinations(self, database):
        ...
        for contig in contigs:
            ...
            annotation = open(f'{self.proj_path}/annotations/{contig}/out_{database}.tsv', 'r')
            annotation_tsv = csv.reader(annotation, delimiter='\t')
            ...
            for row in card_tsv:
                ...
                for row2 in annotation_tsv:
                    ...
                    if combo in combinations_table:
                        ...
                        combinations_table[combo] = 1
                annotation.seek(0) # have to re-seek to start of file for it to work
...
if __name__=="__main__":
    ...
    database = sys.argv[2]
    annotationDvr = AnnotationDriver(proj_path)
    annotationDvr.run_annotation(database)
    annotationDvr.count_combinations(database)
